...
/*async function connect() {
  // Define your provider options if needed (Torus, WalletConnect, etc.)
  const providerOptions = {
    ...
    walletconnect: {
       ...
       options: {
         infuraId: "ea2cacbfec364a7bbc27608bd8e089b3",
       },
    },
    "custom-example": {
      display: {
  ...
  try {
      // Set provider using ethers.js's Web3Provider
      provider = new ethers.providers.Web3Provider(await web3Modal.connect());
      web3 = provider; // Use ethers.js provider as web3

      // Set current network id
      web3gl.networkId = parseInt(await provider.getNetwork().chainId);

      // Check and switch network if needed
      if (web3gl.networkId != window.web3ChainId) {
          try {
              await provider.send("wallet_switchEthereumChain", [{ chainId: `0x${window.web3ChainId.toString(16)}` }]);
          } catch {
              // If network isn't added, pop-up Metamask to add (if using Metamask)
      ...
      web3gl.signer = await provider.getSigner();
      

      // Set current account
      // provider.selectedAddress works for Metamask and some others
      // You may need to handle other wallet providers differently
      web3gl.connectAccount = await provider.getSigner().getAddress();

      console.log("Account:", window.web3gl.connectAccount);
      ...
      // Refresh page if the player changes the account
      provider.on("accountsChanged", (accounts) => {
          window.location.reload();
      });

      // Update if the player changes the network
...
/*async function connect() {
  alert("Ready just wait a few seconds after dismissing this Alert");
    ...
    try {
      ...
      web3gl.connectAccount = await address;
